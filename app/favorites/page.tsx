...
export default function FavoritesPage() {
  ...
  const handleExportCSV = () => {
    ...
    const filename = `propertysignalhq-favorites-${today}.csv`
    const headers  = ['Address', 'City', 'Score', 'Signal Type', 'Date Saved']
    const rows     = favorites.map((f) => [
      f.address,
      f.city || 'this area',
      f.score      ?? '',
      f.signal_type ?? '',
      toCSVDate(f.created_at),
    ])
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#020617] relative overflow-hidden px-4 py-12">
      ...
      <div className="relative z-10 mx-auto max-w-5xl">
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          {/* Sort toggle + Export — only when favorites exist */}
          {favorites.length > 0 && (
