...
function getReply(input: string, isPro: boolean): string | null {
  ...
  if (text.includes('price') || text.includes('cost') || text.includes('$') || text.includes('39')) {
    return 'PropertySignalHQ is $39/month with no hidden fees. Full access to 60+ cities across all 50 states, unlimited searches, and complete property signal data. Most users start finding opportunities within their first session. Cancel anytime. No contracts.'
  }
  if (text.includes('market') || text.includes('city') || text.includes('where') || text.includes('cover')) {
    return 'We cover 60+ cities across all 50 US states including Phoenix, Miami, Dallas, Atlanta, Chicago, Nashville, Denver, Seattle, Charlotte, Boston and many more. Data is enriched weekly with real Zillow market data.'
  }
  ...
  return (
    <div>
      <p className="text-sm leading-relaxed text-gray-300">
        ...
        fees. You get full access to 60+ cities across all 50 states, unlimited searches, and complete property
        signal data.
        <br /><br />
    ...
    <div>
      ...
      <ul className="space-y-1 text-sm text-gray-300">
        {[
          ...
          'Coverage across 60+ cities in all 50 states',
          'Daily updated leads',
        ].map((item) => (
...
function MarketCoverageScreen({ onBack, onClose }: { onBack: () => void; onClose: () => void }) {
  return (
    <div>
      <p className="text-sm leading-relaxed text-gray-300">
        We cover <span className="font-semibold text-white">60+ cities across all 50 US states</span> including
